...
class GlobalWorld{
	public{
		...
		void setLocalWorld(in LocalWorld localWorld, in ar.Vector3i targetPosition){
			grid.setNbhd(localWorld.grid, targetPosition);
		}
...
class WorldRenderer {
	public{
		...
		this(Config config, Network network){
			...
			camera.target = ar.Vector3f(5, 5, 5);
		}

		void draw(){
			// network.getLocalWorld(_localWorld, cast(ar.Vector3i)camera.target, GridRange(ar.Vector3i(-3, -3, -3), ar.Vector3i(3, 3, 3)));
			cellRenderer.draw(_localWorld.grid);
		};
	}//public
